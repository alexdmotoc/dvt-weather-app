...
/// we will receive the forecast for 5 days in 3 hour steps, so we expect to get 40 items (`5 days * 24 hours / 3 hours = 40 steps`)
///
public enum ForecastReducer {
    ...
    /// computing the average for that subarray. The end result is an array where each item represents the forecast for one day.
    ///
    /// - Parameter forecast: a 3 hour step forecast
    /// - Returns: a daily averaged forecast
    ///
    public static func reduceHourlyForecastToDaily(_ forecast: [WeatherInformation.Forecast]) -> [WeatherInformation.Forecast] {
        var startIndex = 0
        ...
        while startIndex < forecast.count {
            ...
            let averageForecast = WeatherInformation.Forecast(currentTemp: averageTemp, weatherType: .init(rawValue: averageWeatherType) ?? .sunny)
